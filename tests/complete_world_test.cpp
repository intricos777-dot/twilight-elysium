...
#include "renderer/model_generator.h"
#include "renderer/complete_world_builder.h"
...
int main(int argc, char** argv) {
    ...
    std::printf("=== Twilight Elysium - Complete World Builder Test ===\n");
    ...
    SDL_Window* window = SDL_CreateWindow(
        "Twilight Elysium - Complete World",
    ...
    // Generate all models for disc1
    auto disc_models = model_gen.generate_disc_models("INFECTION");
    
    // Generate zone-specific models
    auto zone_models = model_gen.generate_zone_models("Mac Anu");
    
    // Generate all models for different zone types
    auto zone_types = {"Mac Anu", "Dun Loireag", "Cave of Trial", "Twilight Palace", 
                       "Dragonbone Wastes", "Kughai Valley"};
    int total_zone_models = 0;
    for (const auto& zn : zone_types) {
        auto zm = model_gen.generate_zone_models(zn);
        total_zone_models += (int)zm.size();
    }
    ...
    std::printf("[Models] disc1=%zu zone_specific=%zu zone_types=%d\n",
        disc_models.size(), zone_models.size(), total_zone_models);
    std::printf("[Models] total_generated=%d\n", model_gen.generated_count());
